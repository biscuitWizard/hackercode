...
import { projectFromSessionContext, projectFromRepository, resolveGitProject } from '../../node/shared/gitProject.js';
...
suite('Copilot Git Project', () => {
	...
	test('falls back to repository context when no git project is available', async () => {
		const project = await projectFromSessionContext({ repository: 'microsoft/vscode' }, gitService);
